class PersonRecordStatus < CouchRest::Model::Base
    ...
    view :by_district_code,
         :map => "function(doc) {
			  if (doc['type'] == 'PersonRecordStatus' && doc['voided'] ==false) {
			    	...
			    	emit(doc['district_code'], 1);
			  }
			}"

    view :by_district_code_and_status,
         :map => "function(doc) {
			  if (doc['type'] == 'PersonRecordStatus' && doc['voided'] ==false) {

			    	emit(doc['district_code'] + '_' + doc['status']);
			  }
			}"
    ...
    view :by_district_code_and_record_status_and_created_at,
         :map => "function(doc) {
			  if (doc['type'] == 'PersonRecordStatus' && doc['voided'] ==false) {
			      var date = new Date(doc['created_at']);
			      var year = date.getFullYear();
			      ...
			      var day = date.getDate();
			      var str = '' + year + '-' + (month > 9 ? month : ('0' + month)) + '-' + (day > 9 ? day : ('0' + day));
			    	emit(doc['district_code'] + '_' + doc['status'] + '_' + str, 1);
			  }
			}"
